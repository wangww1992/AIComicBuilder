...
import { createLanguageModel, extractJSON, stripThinking } from "@/lib/ai/ai-sdk";
...
async function handleScriptOutlineAction(
  ...
  if (boundAgent) {
    try {
      ...
      const saveTransform = new TransformStream<Uint8Array, Uint8Array>({
        ...
        async flush() {
          const outline = stripThinking(outlineBuf).trim();
          if (!outline) return;
          try {
            if (episodeId) {
  ...
  const result = streamText({
    ...
    onFinish: async ({ text }) => {
      try {
        const outline = stripThinking(text).trim();
        if (episodeId) {
          await db
            .update(episodes)
...
async function handleScriptGenerate(
  ...
  if (sgBoundAgent) {
    try {
      ...
      const sgSaveTransform = new TransformStream<Uint8Array, Uint8Array>({
        ...
        async flush() {
          const script = stripThinking(scriptBuf).trim();
          if (!script) return;
          try {
            if (episodeId) {
  ...
  const result = streamText({
    ...
    onFinish: async ({ text }) => {
      try {
        const script = stripThinking(text).trim();
        if (episodeId) {
          await db
            .update(episodes)
...
- 根据优化指令做针对性改进`;
  ...
  if (images.length > 0) {
    ...
    return NextResponse.json({ optimizedText: stripThinking(result).trim() });
  }
...
请输出优化后的文本：`,
  ...
  return NextResponse.json({ optimizedText: stripThinking(text).trim() });
}
